 ...
 */
@Service
public class TemplateNameProviderImpl extends BasePlusProviderImpl<TemplateNameForm> implements TemplateNameProvider {
     ...
     */
    @Override
    public TemplateNameForm create(TemplateNameForm templateNameForm) {
        try {
            // 1、校验入参
            String errorMsg = TemplateNameHandle.checkCreate(templateNameForm);
            if (StringUtils.isNotEmpty(errorMsg)) {
                throw new BusinessException(errorMsg);
            }
            // 2、执行保存方法
            return templateNameService.create(templateNameForm);
        } catch (Exception e) {
